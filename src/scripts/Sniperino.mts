...
import { Adapter, Brain, Response, Robot } from 'hubot';
...
const loadState = (brain: Brain<Adapter>): SniperState => {
    const state = brain.get('sniperino') as SniperState;
    if (state) {
        return state;
    }
...
export default (robot: Robot): void => {
    ...
    robot.on('roll', (...args: unknown[]) => {
        const res = args[0] as Response;
        const roll = args[1] as number;
